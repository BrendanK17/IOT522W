...
export default function Order() {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1">
          {menuItems.map((item) => (
            <Card key={item.id} className="p-4 text-center bg-blue-100 shadow-md">
              ...
              <CardContent>
                <img src={item.photo} className="w-full h-40 object-cover" />
                <p className="font-bold text-lg my-2 text-blue-700">£{item.price.toFixed(2)}</p>
                <Button
                  className="bg-blue-600 hover:bg-blue-700 text-white"
                  onClick={() => addToBasket({ ...item, comment: "", quantity: 1 })}
                >
                  Order
                </Button>
              </CardContent>
      ...
      <aside className="w-80 bg-blue-800 text-white shadow-md p-4 flex flex-col">
        ...
        {basket.length === 0 ? (
          ...
          <ul className="space-y-2">
              ...
              <li key={item.id} className="border-b border-blue-700 pb-2">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex space-x-2">
                    <Button className="bg-red-500 hover:bg-red-600 text-white px-2" onClick={() => decreaseQuantity(item.id)}>
                      -
                    </Button>
                    <Button className="bg-blue-500 hover:bg-blue-600 text-white px-2" onClick={() => increaseQuantity(item.id)}>
                      +
                    </Button>
                    <Button className="bg-red-700 hover:bg-red-800 text-white px-2" onClick={() => removeFromBasket(item.id)}>
                      Delete
                    </Button>
                  </div>
                </div>
            ...
            <p className="font-bold mt-4">Total: £{basket.reduce((acc, item) => acc + item.price * item.quantity, 0).toFixed(2)}</p>
            <Button className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white" onClick={handleCheckout}>
              Proceed to Checkout
            </Button>
          </ul>
        )}
